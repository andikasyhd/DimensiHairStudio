...
export default function ListPelanggan() {
  ...
  const jumlahHariIni = pemesanan.filter(item =>
    new Date(item.created_at).toLocaleDateString('en-CA') === todayDate
  ).length;
  ...
  const handleKirimPesan = async (item, type) => {
    ...
    try {
      ...
      const updatedData = await pemesananAPI.getAllPemesanan();
                    ...
                    <td className="px-4 py-2 space-x-2">
                      <button
                        ...
                        className={`px-3 py-1 rounded text-sm text-white transition ${
                          item.status ? 'bg-green-300 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600'
                        }`}
                      >
                        Setujui
                      </button>
                      <button
                        ...
                        className={`px-3 py-1 rounded text-sm text-white transition ${
                          item.status ? 'bg-red-300 cursor-not-allowed' : 'bg-red-500 hover:bg-red-600'
                        }`}
                      >
                        Tolak
                      </button>
...
//   // ✅ Hitung jumlah pemesanan hari ini berdasarkan created_at (zona waktu lokal)
